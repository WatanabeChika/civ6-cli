...
_GOVERNMENT_STATE = """
local currentCulture = Players[Game.GetLocalPlayer()]:GetCulture()
local okCurrent, currentGovernment = pcall(function() return currentCulture:GetCurrentGovernment() end)
local okAnarchy, inAnarchy = pcall(function() return currentCulture:IsInAnarchy() end)
if okAnarchy and inAnarchy then
    local okTurns, turns = pcall(function() return currentCulture:GetAnarchyTurns() end)
    print("GOVERNMENT_STATE|state=无政府状态|turns=" .. tostring(okTurns and turns or '?') .. "|description=当前没有生效的政体或政策槽位。keep 不会结束无政府状态。")
elseif okCurrent and type(currentGovernment) == 'number' and currentGovernment < 0 then
    print("GOVERNMENT_STATE|state=尚未选择政体|description=当前没有生效的政体；使用 government options 查看可选政体。")
elseif not okCurrent or currentGovernment == nil then
    print("GOVERNMENT_STATE|state=unknown|description=当前政体未知")
end
"""
...
def build_policies_query() -> str:
    ...
    return _LUA_SAFE_READ + _GOVERNMENT_STATE + """
local me = Game.GetLocalPlayer()
local pCulture = Players[me]:GetCulture()
local govIdx = safe(function() return pCulture:GetCurrentGovernment() end,-1)
local govName = "当前没有生效的政体"
local govType = "NONE"
...
local numSlots = safe(function() return pCulture:GetNumPolicySlots() end,-1)
if numSlots < 0 then print('POLICY_NOTE|message=政策槽位信息未知') end
...
for s = 0, numSlots - 1 do
    ...
    local pe = GameInfo.Policies[policyIdx]
    local desc = pe and Locale.Lookup(pe.Description):gsub("|", "/"):gsub("\\n", " ") or "空槽位"
    print("SLOT|" .. s .. "|" .. slotType .. "|" .. policyType .. "|" .. policyName:gsub("|","/") .. "|description=" .. desc)
end
for policy in GameInfo.Policies() do
    local okObsolete, obsolete = pcall(function() return pCulture:IsPolicyObsolete(policy.Hash) end)
    if pCulture:IsPolicyUnlocked(policy.Hash) and not (okObsolete and obsolete) then
        local canSlot = false
        for s = 0, numSlots - 1 do
        ...
        -- With no active government there are no slots, but unlocked cards
        -- remain useful to inspect. Do not misrepresent them as installable.
        if canSlot or numSlots == 0 then
            ...
            print("AVAIL|" .. policy.PolicyType .. "|" .. name:gsub("|","/") .. "|" .. desc .. "|" .. slotType .. "|can_slot=" .. (canSlot and "yes" or "no"))
        end
    end
end
print("{SENTINEL}")
...
def build_keep_government() -> str:
    return f"""
local culture = Players[Game.GetLocalPlayer()]:GetCulture()
-- Opening the Government screen clears this blocker even during anarchy or
-- on builds where GetCurrentGovernment() temporarily returns nil/-1.  The
-- game's own GovernmentScreen uses this setter without requiring a current
-- government, so the CLI follows that contract.
culture:SetGovernmentChangeConsidered(true)
print("OK:GOVERNMENT_KEPT|保留当前政体")
print("{SENTINEL}")
"""


def build_keep_policies() -> str:
    """Confirm the existing full policy layout, without paid UNLOCK_POLICIES."""
    return f"""
local culture = Players[Game.GetLocalPlayer()]:GetCulture()
local clearList, addList = {{}}, {{}}
local okSlots, numSlots = pcall(function() return culture:GetNumPolicySlots() end)
numSlots = okSlots and numSlots or 0
if numSlots <= 0 then
    -- There is no policy layout to change (for example during anarchy).  This
    -- is a valid no-op, not a false NO_GOVERNMENT rejection.
    print("OK:POLICIES_NOT_APPLICABLE|当前没有政策槽位，无需更换政策卡")
    print("{SENTINEL}")
    return
end
for slot = 0, numSlots - 1 do
    local policy = GameInfo.Policies[culture:GetSlotPolicy(slot)]
    if not policy then {_bail("ERR:EMPTY_POLICY_SLOT|Cannot keep an empty layout; use policy options and policy set")} end
    table.insert(clearList, slot)
    addList[slot] = policy.Hash
end
culture:RequestPolicyChanges(clearList, addList)
print("OK:POLICIES_KEPT|确认并保留当前政策卡；未请求付费解锁")
print("{SENTINEL}")
"""
...
local me = Game.GetLocalPlayer()
local pEras = safe(function() return Game.GetEras() end,nil)
if not pEras then print('ERA_NOTE|当前规则无时代着力点系统'); print('---END---'); return end
local age = "Normal"
if safe(function() return pEras:HasHeroicGoldenAge(me) end,false) then age = "Heroic"
elseif safe(function() return pEras:HasGoldenAge(me) end,false) then age = "Golden"
elseif safe(function() return pEras:HasDarkAge(me) end,false) then age = "Dark" end
local era = safe(function() return pEras:GetCurrentEra() end,-1)
local darkT = safe(function() return pEras:GetPlayerDarkAgeThreshold(me) end,-1) or 0
local goldT = safe(function() return pEras:GetPlayerGoldenAgeThreshold(me) end,-1) or 0
local allowed = safe(function() return pEras:GetPlayerNumAllowedCommemorations(me) end,-1)
local score = safe(function() return pEras:GetPlayerCurrentScore(me) end,-1)
local eraRow = era >= 0 and GameInfo.Eras[era] or nil
print("STATUS|" .. age .. "|" .. era .. "|" .. score .. "|" .. darkT .. "|" .. goldT .. "|" .. allowed .. "|era_name=" .. (eraRow and Locale.Lookup(eraRow.Name) or '?'))
local speedType = safe(function() return GameConfiguration.GetGameSpeedType() end,nil)
local foundSpeed = false
pcall(function() for row in GameInfo.GameSpeeds() do
    local hash = safe(function() return GameConfiguration.MakeHash(row.GameSpeedType) end,nil)
    if speedType ~= nil and (row.GameSpeedType == speedType or row.Hash == speedType or hash == speedType) then
        print('GAME_SPEED|' .. row.GameSpeedType .. '|name=' .. Locale.Lookup(row.Name) .. '|cost_multiplier=' .. tostring(row.CostMultiplier or '?'))
        foundSpeed = true
        break
    end
end end)
if not foundSpeed then print('GAME_SPEED|unknown|name=unknown|cost_multiplier=?') end
-- Active commemorations
local active = safe(function() return pEras:GetPlayerActiveCommemorations(me) end,nil)
if active then
...
-- Available choices
local choices = safe(function() return pEras:GetPlayerCommemorateChoices(me) end,nil)
if choices then
...
local me = Game.GetLocalPlayer()
local pCulture = Players[me]:GetCulture()
local curGov = safe(function() return pCulture:GetCurrentGovernment() end,-1)
print('GOVERNMENT_FAVOR|favor=' .. safe(function() return Players[me]:GetFavor() end,'?'))
