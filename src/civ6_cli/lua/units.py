...
from civ6_cli.lua._helpers import (
    _LUA_RES_VISIBLE,
    SENTINEL,
    _bail,
    _bail_lua,
...
-- Active trade routes are not exposed on the unit object.  Build a trader ID
-- set from city route records so their persistent order is shown explicitly.
local routedTraders = {}
...
local function readOrderState(unit, unitType, unitID)
    -- IsReadyToMove is the same per-unit authority used by Civ VI's
    -- ActionPanel/GetFirstReadyUnit flow.  In particular, it is false for
    -- fortified, sleeping, healing, skipped and automated units even when
    -- GetMovesRemaining() is greater than zero.
    local okReady, ready = pcall(function() return unit:IsReadyToMove() end)
    local needsOrders = "unknown"
    if okReady then needsOrders = ready and "yes" or "no" end

    local activityKey = "unknown"
    local okActivity, activity = pcall(function()
        return UnitManager.GetActivityType(unit)
    end)
    if okActivity and activity ~= nil then
        activityKey = "activity_" .. tostring(activity)
        for name, value in pairs(ActivityTypes or {}) do
            if value == activity then
                if name == "NO_ACTIVITY" then
                    activityKey = "none"
                else
                    activityKey = string.lower(string.gsub(name, "^ACTIVITY_", ""))
                end
                break
            end
        end
    ...
    -- These long-running activities can be identified more precisely than
    -- the generic ACTIVITY_OPERATION value. IsAutomated is not exposed in
    -- every ruleset, hence the guarded call.
    local automated = false
    pcall(function() automated = unit:IsAutomated() and true or false end)
    if automated then activityKey = "auto_explore" end
    if routedTraders[unitID] then
        activityKey = "trade_route"
    elseif unitType == "UNIT_SPY" then
        local okSpy, spyOperation = pcall(function() return unit:GetSpyOperation() end)
        if okSpy and spyOperation and spyOperation >= 0 then activityKey = "spy_mission" end
    end

    local fortifyTurns = 0
    pcall(function() fortifyTurns = unit:GetFortifyTurns() or 0 end)
    if activityKey == "sentry" and fortifyTurns > 0 then
        -- The engine does not expose whether this was entered through the
        -- Fortify or Alert button; both are persistent and need no orders.
        activityKey = "fortify_or_alert"
    elseif fortifyTurns > 0 and needsOrders == "no"
        and activityKey ~= "heal" and activityKey ~= "sleep"
        and activityKey ~= "hold" and activityKey ~= "trade_route"
        and activityKey ~= "spy_mission" then
        activityKey = "fortify"
    end

    -- Readiness wins if an alert/fortified unit was woken by an enemy: keeping
    -- the old persistent label in that case would falsely imply no input is
    -- needed even though Civ VI has put the unit back into unit cycling.
    if needsOrders == "yes" then
        activityKey = "ready"
    elseif needsOrders == "no" and (activityKey == "awake" or activityKey == "none") then
        if unit:GetMovesRemaining() <= 0 then activityKey = "exhausted"
        else activityKey = "busy" end
    end
    return activityKey, needsOrders
end

for i, u in Players[id]:GetUnits():Members() do
    local x, y = u:GetX(), u:GetY()
    do -- Travelling spies remain listed; their off-map coordinate is explicit.
        local uid = u:GetID()
        local entry = GameInfo.Units[u:GetType()]
        local ut = entry and entry.UnitType or "UNKNOWN"
        local nm = Locale.Lookup(u:GetName())
        local activity, needsOrders = readOrderState(u, ut, uid)
        ...
        print(uid .. "|" .. (uid % 65536) .. "|" .. nm .. "|" .. ut .. "|" .. x .. "," .. y .. "|" .. u:GetMovesRemaining() .. "/" .. u:GetMaxMoves() .. "|" .. activity .. "|" .. needsOrders .. "|" .. (u:GetMaxDamage() - u:GetDamage()) .. "/" .. u:GetMaxDamage() .. "|" .. cs .. "|" .. rs .. "|" .. charges .. "|" .. targets .. "|" .. promo .. "|" .. canUp .. "|" .. upName .. "|" .. upCost .. "|" .. validImps .. "|" .. relName)
    end
end
print("{SENTINEL}")
...
# Operations whose complete parameter contract is a target plot (or current plot).
# Operations with a different contract use dedicated commands; for example,
# Apostle evangelization is exposed through ``religion evangelize`` and
# ``religion add-belief`` rather than being forced through this plot-task API.
PLOT_TASKS = (
    "EXCAVATE", "DESIGNATE_PARK", "TOURISM_BOMB", "LAUNCH_INQUISITION",
    "REMOVE_HERESY", "RELIGIOUS_HEAL", "REBASE", "DEPLOY", "PILLAGE",
    "PILLAGE_ROUTE", "COASTAL_RAID", "PLANT_FOREST", "HARVEST_RESOURCE",
    "CLEAR_CONTAMINATION", "REPAIR_ROUTE", "CONVERT_BARBARIANS",
)


def build_unit_operations_query(unit_index: int) -> str:
    """Native visible operation catalogue for one owned unit, not debug ops."""
    tasks = ",".join(f'["UNITOPERATION_{name}"]=true' for name in PLOT_TASKS)
    return f"""
{_lua_get_unit(unit_index)}
local info = GameInfo.Units[unit:GetType()]
local typ = info and info.UnitType or "UNKNOWN"
local mode = info and info.MakeTradeRoute and "商人：迁移到城市或建立商路"
    or typ == "UNIT_SPY" and "间谍：派遣到城市并选择任务"
    or info and info.Domain == "DOMAIN_AIR" and "空军：转场、部署或空袭"
    or "可自由移动；专业动作需位于合适目标地块"
print("UNIT_MODE|id=" .. unit:GetID() .. "|type=" .. typ .. "|message=" .. mode)
local plot = Map.GetPlot(unit:GetX(), unit:GetY())
local supported = {{{tasks}}}
local commands = {{MOVE_TO="move", RANGE_ATTACK="attack", AIR_ATTACK="attack", FORTIFY="fortify",
    HEAL="heal", REST_REPAIR="heal", ALERT="alert", SLEEP="sleep", SKIP_TURN="skip",
    AUTOMATE_EXPLORE="explore", FOUND_CITY="found-city", REMOVE_FEATURE="remove-feature",
    REPAIR="repair", REMOVE_IMPROVEMENT="remove-improvement", BUILD_ROUTE="build-route",
    SPREAD_RELIGION="spread-religion", UPGRADE="upgrade"}}
local operations, seen = {{}}, {{}}
local function add(row)
    if row and row.VisibleInUI and not seen[row.OperationType] then
        seen[row.OperationType] = true
        table.insert(operations, row)
    end
end
for row in GameInfo.UnitOperations() do add(row) end
-- Supplement iteration when the tuner exposes only direct lookups.
for name in pairs(commands) do add(GameInfo.UnitOperations["UNITOPERATION_" .. name]) end
for name in pairs(supported) do add(GameInfo.UnitOperations[name]) end
for _, name in ipairs({{"BUILD_IMPROVEMENT", "EVANGELIZE_BELIEF", "FOUND_RELIGION", "MAKE_TRADE_ROUTE", "TELEPORT_TO_CITY"}}) do
    add(GameInfo.UnitOperations["UNITOPERATION_" .. name])
end
table.sort(operations, function(a,b) return a.OperationType < b.OperationType end)
for _, row in ipairs(operations) do
    if not row.OperationType:find("UNITOPERATION_SPY_", 1, true) then
        local ok, can, results = pcall(function() return UnitManager.CanStartOperation(unit, row.Hash, nil, true) end)
        if ok and can then
            -- The UI check above only determines whether this unit type can
            -- ever perform the action. The strict check describes readiness.
            local strictOK, available, strictResults = pcall(function()
                return UnitManager.CanStartOperation(unit, row.Hash, nil, false, true)
            end)
            if strictOK then results = strictResults end
            local suffix = row.OperationType:gsub("UNITOPERATION_", "")
            local command = commands[suffix] and "unit " .. commands[suffix] .. " " .. unit:GetID() or ""
            if commands[suffix] == "move" or commands[suffix] == "attack" then command = command .. " <x> <y>" end
            if supported[row.OperationType] then command = "unit task " .. unit:GetID() .. " " .. row.OperationType .. " [x y]" end
            if suffix == "MAKE_TRADE_ROUTE" then command = "unit options " .. unit:GetID() .. " / unit trade-route " .. unit:GetID() .. " <x> <y>" end
            if suffix == "TELEPORT_TO_CITY" then command = "unit teleport " .. unit:GetID() .. " <x> <y>" end
            if suffix == "BUILD_IMPROVEMENT" then command = "unit improve " .. unit:GetID() .. " <IMPROVEMENT_TYPE>" end
            if suffix == "FOUND_RELIGION" then command = "religion belief-options / religion found ..." end
            if suffix == "EVANGELIZE_BELIEF" then command = "religion evangelize " .. unit:GetID() .. "；之后 religion belief-options / religion add-belief <BELIEF_TYPE>" end
            if command == "" then command = "需要额外选择；请在游戏界面完成" end
            local targets = {{}}
            if results and results[UnitOperationResults.PLOTS] then
                for _, id in ipairs(results[UnitOperationResults.PLOTS]) do
                    local target = Map.GetPlotByIndex(id)
                    if target then table.insert(targets, target:GetX() .. "," .. target:GetY()) end
                end
            end
            local reasons = {{}}
            if results and results[UnitOperationResults.FAILURE_REASONS] then
                for _, reason in ipairs(results[UnitOperationResults.FAILURE_REASONS]) do table.insert(reasons, Locale.Lookup(reason)) end
            end
            if row.InterfaceMode and #targets == 0 then
                pcall(function()
                    local result = UnitManager.GetOperationTargets(unit, row.Hash)
                    for _, id in ipairs(result and result[UnitOperationResults.PLOTS] or {{}}) do
                        local target = Map.GetPlotByIndex(id)
                        if target then table.insert(targets, target:GetX() .. "," .. target:GetY()) end
                    end
                end)
            end
            local availability = strictOK and (available and "yes" or "no") or "unknown"
            if suffix == "MAKE_TRADE_ROUTE" or suffix == "TELEPORT_TO_CITY" or (row.InterfaceMode and #targets > 0) then
                availability = "target_required"
            end
            print("UNIT_OPERATION|type=" .. row.OperationType .. "|name=" .. Locale.Lookup(row.Description):gsub("|", "/")
                .. "|available=" .. availability
                .. "|requirements=" .. table.concat(reasons, "; "):gsub("|", "/")
                .. "|targets=" .. table.concat(targets, "; ") .. "|next=" .. command)
        end
    end
end
-- Persistent activity controls are commands, not UnitOperations.
for _, name in ipairs({{"WAKE", "CANCEL", "ACTIVATE_GREAT_PERSON", "UPGRADE", "DELETE"}}) do
    local row = GameInfo.UnitCommands["UNITCOMMAND_" .. name]
    local hash = row and row.Hash or UnitCommandTypes[name]
    if hash then
        local ok, visible = pcall(function() return UnitManager.CanStartCommand(unit, hash, true) end)
        if ok and visible then
            local strictOK, available = pcall(function() return UnitManager.CanStartCommand(unit, hash, false, true) end)
            local verbs = {{WAKE="wake", CANCEL="cancel", ACTIVATE_GREAT_PERSON="activate", UPGRADE="upgrade", DELETE="disband"}}
            print("UNIT_OPERATION|type=UNITCOMMAND_" .. name .. "|name=" .. (row and Locale.Lookup(row.Description):gsub("|", "/") or name)
                .. "|available=" .. (strictOK and (available and "yes" or "no") or "unknown")
                .. "|next=unit " .. verbs[name] .. " " .. unit:GetID())
        end
    end
end
print("{SENTINEL}")
...
def build_unit_control(unit_index: int, command: str) -> str:
    if command not in {"WAKE", "CANCEL"}:
        raise ValueError("Unsupported activity control")
    return f"""
{_lua_get_unit(unit_index)}
local row = GameInfo.UnitCommands["UNITCOMMAND_{command}"]
local hash = row and row.Hash or UnitCommandTypes.{command}
if not hash then {_bail("ERR:COMMAND_UNAVAILABLE|Activity control unavailable in this ruleset")} end
if not UnitManager.CanStartCommand(unit, hash, false, true) then {_bail("ERR:CANNOT_CONTROL|Unit cannot wake/cancel in its current state")} end
UnitManager.RequestCommand(unit, hash)
print("OK:{command}_REQUESTED|unit=" .. unit:GetID())
print("{SENTINEL}")
"""


def build_plot_task(unit_index: int, operation: str, x: int | None = None, y: int | None = None) -> str:
    suffix = operation.removeprefix("UNITOPERATION_")
    if suffix not in PLOT_TASKS:
        raise ValueError("此任务需要专用参数入口；用 unit options 查看可用命令")
    target_x = "unit:GetX()" if x is None else str(x)
    target_y = "unit:GetY()" if y is None else str(y)
    return f"""
{_lua_get_unit(unit_index)}
local operation = GameInfo.UnitOperations["UNITOPERATION_{suffix}"]
if not operation then {_bail("ERR:OPERATION_NOT_FOUND|Operation unavailable in this ruleset")} end
local x, y = {target_x}, {target_y}
local plot = Map.GetPlot(x, y)
if not plot then {_bail("ERR:INVALID_TARGET|Target plot does not exist")} end
local params = {{[UnitOperationTypes.PARAM_X]=x, [UnitOperationTypes.PARAM_Y]=y}}
local can = UnitManager.CanStartOperation(unit, operation.Hash, plot, params)
if not can then {_bail("ERR:CANNOT_TASK|Operation not legal for this unit and target; read unit options first")} end
UnitManager.RequestOperation(unit, operation.Hash, params)
print("OK:TASK_REQUESTED|unit=" .. unit:GetID() .. "|UNITOPERATION_{suffix}|target=" .. x .. "," .. y)
print("{SENTINEL}")
"""
...
def build_move_unit(unit_index: int, target_x: int, target_y: int) -> str:
    return f"""
{_lua_get_unit(unit_index)}
local movingInfo = GameInfo.Units[unit:GetType()]
if movingInfo and (movingInfo.MakeTradeRoute or movingInfo.UnitType == "UNIT_SPY" or movingInfo.Domain == "DOMAIN_AIR") then
    {_bail("ERR:TASK_MOVEMENT|Use unit options: traders use teleport/trade-route, spies use travel/mission, aircraft use REBASE/DEPLOY")}
end
...
if u then
    local name, moves, hp, ready = "?", "?", "?", "unknown"
    pcall(function() name = Locale.Lookup(u:GetName()):gsub("|", "/") end)
    pcall(function() moves = u:GetMovesRemaining() .. "/" .. u:GetMaxMoves() end)
    pcall(function() hp = (u:GetMaxDamage() - u:GetDamage()) .. "/" .. u:GetMaxDamage() end)
    pcall(function() ready = u:IsReadyToMove() and "yes" or "no" end)
    print("UNIT_STATE|id=" .. u:GetID() .. "|name=" .. name .. "|moves=" .. moves .. "|hp=" .. hp .. "|needs_orders=" .. ready)
end
...
def build_fortify_remaining_units() -> str:
    """Fortify/heal combat units that the game says need orders (InGame context).
...
for _, unit in Players[me]:GetUnits():Members() do
    local x = unit:GetX()
    local okReady, ready = pcall(function() return unit:IsReadyToMove() end)
    if x ~= -9999 and okReady and ready then
        local info = GameInfo.Units[unit:GetType()]
...
def build_skip_remaining_units() -> str:
    """Skip only units that the game says need orders (GameCore context)."""
    return """
local me = Game.GetLocalPlayer()
local count = 0
for _, unit in Players[me]:GetUnits():Members() do
