...
from civ6_cli.lua.models import TradeDestination, TraderInfo, TradeRouteStatus
...
if not unitInfo or not unitInfo.MakeTradeRoute then {_bail("ERR:NOT_TRADER|This unit cannot establish trade routes")} end
...
local function enrichDest(i, city, cx, cy, isDom)
    ...
    pcall(function()
        local path = tm:GetTradeRoutePath(me, origCID, i, city:GetID())
        if path and #path > 1 then
            distance = #path - 1
            local multiplier = 100
            local speedType = GameConfiguration.GetGameSpeedType()
            for speed in GameInfo.GameSpeeds() do
                if speed.Hash == speedType or speed.GameSpeedType == speedType then multiplier = speed.CostMultiplier end
            end
            local parameter = GameInfo.GlobalParameters["TRADE_ROUTE_TURN_DURATION_BASE"]
            local base = math.ceil((parameter and tonumber(parameter.Value) or 20) * multiplier / 100)
            local roundTrip = distance * 2
            -- Routes finish on a return to origin after the minimum duration.
            duration = (math.floor(base / roundTrip) + 1) * roundTrip
        end
    end)
    print("TDEST|" .. Locale.Lookup(city:GetName()):gsub("|", "/") .. "|" .. civ .. "|" .. cx .. "," .. cy .. "|" .. (isDom and "1" or "0") .. "|" .. (isCS and "1" or "0") .. "|" .. (hasQ and "1" or "0") .. "|" .. (hasTP and "1" or "0") .. "|" .. pOut .. "|" .. relName .. "|" .. pIn .. "|" .. relIn .. "|" .. oy .. "|" .. dy .. "|" .. distance .. "|" .. duration)
end
local found = 0
for i = 0, 62 do
...
if found == 0 then
    ...
    -- Do not present unchecked cities as legal routes when no route can start.
end
print("{SENTINEL}")
"""
            ...
            if len(parts) >= 14:
                ...
                results.append(
                    TradeDestination(
                        ...
                        one_way_distance=int(parts[14]) if len(parts) > 14 and parts[14].isdecimal() else None,
                        estimated_turns=int(parts[15]) if len(parts) > 15 and parts[15].isdecimal() else None,
                    )
                )
            elif len(parts) >= 5:
