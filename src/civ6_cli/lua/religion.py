...
from civ6_cli.lua._helpers import SENTINEL, _bail, _bail_lua, _lua_get_unit
...
def build_evangelize_belief(unit_index: int) -> str:
    """Use an Apostle's evangelize operation and create one pending belief choice.

    Civ VI implements this as two distinct UI stages: the unit operation earns
    the belief, then ``PlayerOperations.ADD_BELIEF`` commits the selected
    belief.  Keeping those stages separate avoids consuming an Apostle before
    an invalid belief has been inspected.
    """
    return f"""
{_lua_get_unit(unit_index)}
local info = GameInfo.Units[unit:GetType()]
if not info or not info.EvangelizeBelief then
    {_bail("ERR:NOT_EVANGELIST|Selected unit cannot evangelize a belief")}
end
local religionType = Players[me]:GetReligion():GetReligionTypeCreated()
if religionType == nil or religionType < 0 then
    {_bail("ERR:NO_FOUNDED_RELIGION|Found a religion before evangelizing a belief")}
end
local operation = GameInfo.UnitOperations["UNITOPERATION_EVANGELIZE_BELIEF"]
if not operation then {_bail("ERR:OPERATION_UNAVAILABLE|EVANGELIZE_BELIEF is unavailable in this ruleset")} end
local ok, canStart, results = pcall(function()
    return UnitManager.CanStartOperation(unit, operation.Hash, nil, false, true)
end)
if not ok or not canStart then
    local reasons = {{}}
    if results and results[UnitOperationResults.FAILURE_REASONS] then
        for _, reason in ipairs(results[UnitOperationResults.FAILURE_REASONS]) do
            table.insert(reasons, Locale.Lookup(reason):gsub("|", "/"))
        end
    end
    {_bail_lua('"ERR:CANNOT_EVANGELIZE|" .. (#reasons > 0 and table.concat(reasons, "; ") or "The Apostle is not currently eligible")')}
end
local unitName = Locale.Lookup(unit:GetName()):gsub("|", "/")
UnitManager.RequestOperation(unit, operation.Hash)
print("OK:EVANGELIZED|" .. unitName
    .. " 已获得一个待选信条；下一步：religion belief-options，然后 religion add-belief <BELIEF_TYPE>")
print("{SENTINEL}")
"""


def build_add_belief(belief_type: str) -> str:
    """Choose a pending earned belief using the same operation as ReligionScreen."""
    return f"""
local me = Game.GetLocalPlayer()
local playerReligion = Players[me]:GetReligion()
local religionType = playerReligion:GetReligionTypeCreated()
if religionType == nil or religionType < 0 then
    {_bail("ERR:NO_FOUNDED_RELIGION|You do not have a founded religion")}
end
local belief = GameInfo.Beliefs["{belief_type}"]
if not belief then {_bail(f"ERR:BELIEF_NOT_FOUND|{belief_type}")} end
if belief.BeliefClassType == "BELIEF_CLASS_PANTHEON" then
    {_bail(f"ERR:INVALID_BELIEF_CLASS|{belief_type} is a pantheon, not a religion belief")}
end
local gameReligion = Game.GetReligion()
if gameReligion:IsInSomePantheon(belief.Index) or gameReligion:IsInSomeReligion(belief.Index) then
    {_bail(f"ERR:BELIEF_TAKEN|{belief_type} is already used")}
end
if gameReligion:IsTooManyForReligion(belief.Index, religionType) then
    {_bail(f"ERR:BELIEF_CLASS_FILLED|Your religion cannot add {belief_type}")}
end
local equipped = 0
for _, religion in ipairs(gameReligion:GetReligions()) do
    if religion.Founder == me then
        for _ in pairs(religion.Beliefs or {{}}) do equipped = equipped + 1 end
        break
    end
end
local earned = playerReligion:GetNumBeliefsEarned()
if earned <= equipped then
    {_bail("ERR:NO_PENDING_BELIEF|Use religion evangelize <使徒> before selecting another belief")}
end
local params = {{}}
params[PlayerOperations.PARAM_BELIEF_TYPE] = belief.Hash
params[PlayerOperations.PARAM_INSERT_MODE] = PlayerOperations.VALUE_EXCLUSIVE
UI.RequestPlayerOperation(me, PlayerOperations.ADD_BELIEF, params)
print("OK:BELIEF_ADDED|{belief_type}|" .. Locale.Lookup(belief.Name):gsub("|", "/"))
print("{SENTINEL}")
"""
