...
from civ6_cli.lua.models import CongressProposal, CongressResolution, WorldCongressStatus
...
local wc = safe(function() return Game.GetWorldCongress() end,nil)
if not wc then {_bail("CONGRESS_NOTE|当前规则或时代尚无世界议会")} end
local inSession = safe(function() return wc:IsInSession() end,false)
local meeting = safe(function() return wc:GetMeetingStatus() end,nil)
local turnsLeft = meeting and meeting.TurnsLeft or -1
local favor = safe(function() return Players[me]:GetFavor() end,-1)
local costs = safe(function() return wc:GetVotesandFavorCost() end,{{}})
local maxVotes = costs.MaxVotes or -1
if maxVotes < 0 then print('CONGRESS_NOTE|可投票数与票价未知') end
local costStr = ""
for i = 0, maxVotes do
    if i > 0 then costStr = costStr .. "," end
    costStr = costStr .. tostring(costs[i] or -1)
end
print("WC_STATUS|" .. tostring(inSession) .. "|" .. turnsLeft .. "|" .. favor .. "|" .. maxVotes .. "|" .. costStr)
local ress = safe(function() return wc:GetResolutions() end,nil)
if ress then
...
if inSession then
    local props = safe(function() return wc:GetProposals() end,nil)
    if props then
