...
from civ6_cli.lua._helpers import SENTINEL
from civ6_cli.lua.models import GameNotification
        ...
        if entry and not entry:IsDismissed() then
            ...
            if bt and bt ~= 0 then
                ...
                if not seen[typeName] then
                    ...
                    if typeName:find("DISLOYAL_CITY", 1, true) or typeName:find("REBELLED_CITY", 1, true)
                        or typeName:find("RAZE_CITY", 1, true) or typeName:find("FREE_CITY", 1, true)
                        or typeName:find("CITY_CAPTURE", 1, true) then
                        local source = (typeName:find("DISLOYAL_CITY", 1, true) or typeName:find("REBELLED_CITY", 1, true)) and "rebelled" or "captured"
                        local city = source == "rebelled" and Players[me]:GetCities():GetNextRebelledCity()
                            or Players[me]:GetCities():GetNextCapturedCity()
                        if not city then
                            pcall(function()
                                local x, y = entry:GetLocation()
                                if x and y and x >= 0 and y >= 0 then city = CityManager.GetCityAt(x, y) end
                            end)
                        end
                        if city and city:GetOwner() == me then
                            local choices = source == "rebelled" and "keep, reject"
                                or "keep, raze, liberate-founder, liberate-previous"
                            print("BLOCKING_CITY|id=" .. city:GetID()
                                .. "|name=" .. Locale.Lookup(city:GetName()):gsub("|", "/")
                                .. "|population=" .. city:GetPopulation() .. "|source=" .. source
                                .. "|choices=" .. choices)
                        end
                    end
                    found = found + 1
