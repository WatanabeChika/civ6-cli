...
from civ6_cli.lua.models import CityInfo, ProductionOption


_PENDING_CAPTURE_NOTIFICATIONS = """
-- KEEP is legal even for ordinary cities in some builds: never use it as a
-- pending-state detector. Capture/loyalty decision notifications identify all
-- queued target locations, including cities after the two 'next' pointers.
local notificationList = NotificationManager.GetList(me) or {}
for _, notificationID in ipairs(notificationList) do
    pcall(function()
        local entry = NotificationManager.Find(me, notificationID)
        if not entry or entry:IsDismissed() then return end
        local blocking = entry:GetEndTurnBlocking()
        if not blocking or blocking == 0 then return end
        local kind = entry:GetTypeName() or ""
        for name, value in pairs(EndTurnBlockingTypes or {}) do
            if value == blocking then kind = kind .. " " .. name; break end
        end
        if kind:find("RAZE_CITY", 1, true) or kind:find("FREE_CITY", 1, true)
            or kind:find("REBELLED_CITY", 1, true) or kind:find("CITY_CAPTURE", 1, true)
            or kind:find("DISLOYAL_CITY", 1, true)
            or kind:find("KEEP_CITY", 1, true) or kind:find("ACCEPT_CITY", 1, true) then
            local x, y = entry:GetLocation()
            if x and y and x >= 0 and y >= 0 then
                local target = CityManager.GetCityAt(x, y)
                if target and target:GetOwner() == me then
                    add(target, kind:find("RAZE_CITY", 1, true) and "captured" or "rebelled")
                end
            end
        end
    end)
...
for i, c in Players[me]:GetCities():Members() do
    ...
    if bq:GetSize() > 0 then
        ...
        if h == 0 then
            ...
            -- A query must not edit the queue, even when its entry is obsolete.
            producing = "nothing"
        else
            producing = hashName[h] or "UNKNOWN"
        end
        turnsLeft = safe(function() return bq:GetTurnsLeft() end,-1)
    end
    local g = c:GetGrowth()
    local amNeed = 0
...
def build_resolve_city_capture(action: str, city_id: int | None = None) -> str:
    """InGame context: resolve a 'Keep or Free City' / 'Raze City' blocker.

    action: 'keep', 'reject', 'raze', 'liberate_founder', 'liberate_previous'
    Uses both queue heads plus active decision notification locations.
    """
    directive_map = {
    ...
    selector = "nil" if city_id is None else str(city_id)
    return f"""
local me = Game.GetLocalPlayer()
local player = Players[me]
local pending = {{}}
local seen = {{}}
local function add(city, source)
    if city and not seen[city:GetID()] then
        seen[city:GetID()] = true
        table.insert(pending, {{city=city, source=source}})
    end
end
add(player:GetCities():GetNextRebelledCity(), "rebelled")
add(player:GetCities():GetNextCapturedCity(), "captured")
{_PENDING_CAPTURE_NOTIFICATIONS}
local selectedID = {selector}
if selectedID == nil and #pending > 1 then {_bail("ERR:CITY_REQUIRED|Multiple pending cities; use city capture <city> <action>")} end
local city, source = nil, nil
for _, entry in ipairs(pending) do
    if selectedID == nil or entry.city:GetID() == selectedID or entry.city:GetID() == selectedID % 65536 then
        city, source = entry.city, entry.source
        break
    end
end
...
if source == "rebelled" and "{action}" ~= "keep" and "{action}" ~= "reject" then
    {_bail("ERR:INVALID_REBELLED_DECISION|A loyalty-flipped city offers only keep/reject")} end
local params = {{}}
...
for unit in GameInfo.Units() do
    if bq:CanProduce(unit.Hash, true) then
        if unit.UnitType == "UNIT_TRADER" and traderCapped then
            ...
            if canStart then
                local t = safe(function() return bq:GetTurnsLeft(unit.Hash) end,-1)
...
for bldg in GameInfo.Buildings() do
    if bq:CanProduce(bldg.Hash, true) then
        ...
        if canStart then
            local t = safe(function() return bq:GetTurnsLeft(bldg.Hash) end,-1)
            local gc = -1
...
for dist in GameInfo.Districts() do
    if bq:CanProduce(dist.Hash, true) then
        local t = safe(function() return bq:GetTurnsLeft(dist.Hash) end,-1)
...
for proj in GameInfo.Projects() do
    if bq:CanProduce(proj.Hash, true) then
        local t = safe(function() return bq:GetTurnsLeft(proj.Hash) end,-1)
...
for _, d in pCity:GetDistricts():Members() do
    if d:IsPillaged() then
        ...
        if dInfo and dInfo.DistrictType ~= "DISTRICT_CITY_CENTER" then
            ...
            if canRepair then
                local t = safe(function() return bq:GetTurnsLeft(dInfo.Hash) end,-1)
...
if not canStart then
    -- Only inspect extra diagnostics on failure. Once a request succeeds we
    -- deliberately avoid GetTurnsLeft/CurrentlyBuilding readback because
    -- those optional bindings are unimplemented on some Civ VI builds.
    local pillaged = {{}}
    for _, d in pCity:GetDistricts():Members() do
        if d:IsPillaged() then
            local dInfo = GameInfo.Districts[d:GetType()]
    ...
    if #pillaged > 0 then
        {_bail_lua(f'"ERR:CANNOT_START_PRODUCTION|{item_name}|PILLAGED:" .. table.concat(pillaged, ",")')}
    end
    {_bail(f"ERR:CANNOT_START_PRODUCTION|{item_name}")}
end
local tParams = {{}}
...
print("OK:PRODUCING|{item_name}|请求已由游戏接受")
print("{SENTINEL}")
"""
...
if cur == "{item_name}" then
    -- GameCore does not implement the zero-argument GetTurnsLeft overload.
    -- Optional turn estimates must never invalidate a confirmed queue change.
    local item = GameInfo.Units["{item_name}"] or GameInfo.Buildings["{item_name}"]
        or GameInfo.Districts["{item_name}"] or GameInfo.Projects["{item_name}"]
    local ok, turns = pcall(function() return bq:GetTurnsLeft(item.Hash) end)
    print("CONFIRMED|{item_name}|" .. (ok and turns and turns >= 0 and tostring(turns) .. " turns" or "项目已设置；回合估计不可用"))
else
...
def build_pending_captures_query() -> str:
    """Only list decisions the engine currently exposes; never ordinary cities."""
    return f"""
local me = Game.GetLocalPlayer()
local cities = Players[me]:GetCities()
local seen = {{}}
local function show(city, source)
    if not city or seen[city:GetID()] then return end
    seen[city:GetID()] = true
    local choices = {{}}
    local directives = {{keep=CityDestroyDirectives.KEEP, reject=CityDestroyDirectives.REJECT,
        raze=CityDestroyDirectives.RAZE, ["liberate-founder"]=CityDestroyDirectives.LIBERATE_FOUNDER,
        ["liberate-previous"]=CityDestroyDirectives.LIBERATE_PREVIOUS_OWNER}}
    for name, flag in pairs(directives) do
        local params = {{[UnitOperationTypes.PARAM_FLAGS]=flag}}
        local ok, can = pcall(function() return CityManager.CanStartCommand(city, CityCommandTypes.DESTROY, params) end)
        if ok and can and (source ~= "rebelled" or name == "keep" or name == "reject") then table.insert(choices, name) end
    end
    table.sort(choices)
    print("CAPTURE_OPTION|id=" .. city:GetID() .. "|name=" .. Locale.Lookup(city:GetName()):gsub("|", "/")
        .. "|population=" .. city:GetPopulation() .. "|source=" .. source .. "|choices=" .. table.concat(choices, ", "))
end
local add = show
show(cities:GetNextRebelledCity(), "rebelled")
show(cities:GetNextCapturedCity(), "captured")
{_PENDING_CAPTURE_NOTIFICATIONS}
print("{SENTINEL}")
"""
...
def build_purchase_item(
    ...
    if itype == "DISTRICT" and (target_x is None or target_y is None):
        return _bail(f"ERR:MISSING_COORDS|{item_name} is a district and requires target_x/target_y")
    xy_params = ""
    if target_x is not None and target_y is not None:
        xy_params = (
